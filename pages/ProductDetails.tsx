...
import { Minus, Plus, ShoppingBag, Check, AlertCircle, ChevronDown, ChevronUp, Truck, ShieldCheck, Share2, ArrowRight, ArrowLeft } from 'lucide-react';
...
const ProductDetails: React.FC<ProductDetailsProps> = ({ products, onAddToCart }) => {
  ...
  const uniqueSizes = Array.from(new Set(product.variants.map(v => v.size))) as string[];
  const uniqueColors = Array.from(new Set(product.variants.map(v => v.color))) as string[];
